...
#$ -pe smp 5         		 # Request CPU cores
#$ -l h_rt=72:0:0	 		 # Request hours runtime (upto 240 hours)
#$ -l h_vmem=3G      		 # Request GB RAM / core, total= cores * vmem
...
cds=${root}FRAX09_large_72hr/cds-parts.fasta
echo ${cds}
...
tblastn -num_threads 5 -query ${cds} -db ${out}/blastdb -evalue 1e-5 -out ${out}/tblastn.txt -outfmt "6 std sallseqid score nident positive gaps ppos qframe sframe qseq sseq qlen slen salltitles" -db_gencode 1 -matrix BLOSUM62 -seg no -word_size 3 -comp_based_stats F -gapopen 11 -gapextend 1

echo "GeMoMa:~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
java -jar ${GEMOMA} CLI GeMoMa t=${out}/tblastn.txt c=${cds} a=${root}/assignment.tabular tg=${target} outdir=${out}
